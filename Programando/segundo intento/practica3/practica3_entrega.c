...
///////////SI EL CAMPO ES EMAIL
    ...
    for(i=0; i < n_amigos; i++){
       lista_dos = lista_amigos[i]; //Lee amigo por amigo
...
for(int i=0; i<n_amigos; i++){
    lista_nueva=lista_amigos[i]; //Leemos amigo por amigo
    while(cadena[i]=='1'){
        printf("\n%s",lista_nueva.direcc.pais); //Cogemos el primer pais
        for(int a=0; a<n_amigos; a++){
            lista_nueva2=lista_amigos[a];
            if(strcmp(lista_nueva2.direcc.pais,lista_nueva.direcc.pais)==0 && cadena[a]=='1'){ //Si coinciden los paises
                cadena[a]='0';
                printf("\n                    %d",lista_nueva2.direcc.c_postal); //Imprimimos el codigo postal y el nombre
                printf("\n                                        %s",lista_nueva2.nombre);
                for(int m=a+1; m<n_amigos; m++){ //Recorremos la lista buscando un codigo postal igual
                    ...
                    if(strcmp(lista_nueva3.direcc.pais,lista_nueva.direcc.pais)==0 && cadena[m]=='1'){
                        if(lista_nueva3.direcc.c_postal==lista_nueva2.direcc.c_postal){
                            cadena[m]='0';
                            printf("\n                                        %s",lista_nueva3.nombre); //Si coinciden los codigos postales y el pais imprimimos el nombre
